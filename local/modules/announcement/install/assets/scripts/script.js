$(function() {
    ...
    let params = $('#announcement-params').data().params;
    ...
    for (let pair of options.entries()) {
        html += '<div class="sidebar-widget-item --with-separator announcement-item">';
        html += '<div class="task-item announcement-task">';
        if (params.icons == 'Y') {
            html += '<span class="task-item-counter-wrap"><span class="task-item-counter b24-app-icon b24-app-icon-' + icons[i] + '"></span></span>';
        }
        html += '<span class="task-item-text announcement-text" style="color:' + params.text_color + ';">' + pair[0] + '</span>';
        html += '<span class="task-item-index announcement-text"> ' + pair[1];
        html += '</span></div></div>';
        i++;
    }
