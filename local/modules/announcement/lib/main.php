...
class main {
    ...
    {
        if (Option::get($this->moduleId, 'switch_on', 'Y') == 'Y') {
            global $APPLICATION;

            //if ($APPLICATION->GetCurPage() == '/stream/') {
                if ($this->isUserPermission()) {
                    Asset::getInstance()->addCss('/bitrix/css/'.$this->moduleId.'/style.css');
                    
                    if (Option::get($this->moduleId, 'jquery_on', 'Y') == 'Y') {
                        \CJSCore::init(array('jquery3'));
                    }

                    Asset::getInstance()->addString(
                        "<script id='".$this->moduleId."-params' data-params='".$this->getJsonDataWeather()."'></script>",
                        true
                    );

                    Asset::getInstance()->addJs('/bitrix/js/'.$this->moduleId.'/script.js');
                } else {                    
                    return false;
                }          
            //}
