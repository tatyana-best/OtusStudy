...
<span class="fields enumeration field-wrap" data-has-input="no">
	...
	elseif($arResult['userField']['SETTINGS']['DISPLAY'] === ElementType::DISPLAY_DIALOG)
	{
		...
		<?php
        \Bitrix\Main\UI\Extension::load('bestrank.crmdependentfields.userfield-selector');
		$params = \Bitrix\Main\Web\Json::encode([
            ...
            'filter' => $arResult['DISPLAY_FILTER_ELEMENTS']
		]);

		$script = <<<EOT
		<script>
			BX.ready(() => {
				const elementSelector = new BX.CrmDependentfields.UserFieldSelector('{$params}');
...
EOT;
	...
	elseif($arResult['userField']['SETTINGS']['DISPLAY'] === ElementType::DISPLAY_UI)
	{
		...
		>
			<?php
			if(is_array($arResult['attrList'])&&count($arResult['attrList']))
			{
